    ...
    <section class="bg-qaari-bg px-4 pb-16 pt-28 sm:px-6">
        <div class="mx-auto max-w-3xl">
            <div class="mb-8 flex flex-wrap items-end justify-between gap-4">
                ...
                <a href="{{ route('library.playlists') }}" class="text-sm font-semibold text-qaari-primary hover:text-qaari-accent">
                    {{ __('site.playlists') }} →
                </a>
            </div>

            @if ($favorites->isEmpty())
                <p class="text-qaari-muted">{{ __('site.favorites_empty') }}</p>
                <a href="{{ route('reciters.index') }}" class="mt-4 inline-flex rounded-full bg-qaari-primary px-5 py-2.5 text-sm font-semibold text-qaari-primary-fg">
                    {{ __('site.browse') }}
                </a>
            @else
                <ul class="space-y-2">
                    @foreach ($favorites as $track)
                        ...
                        <li class="flex items-center gap-3 rounded-2xl bg-qaari-card px-4 py-3 ring-1 ring-qaari-border">
                            ...
                            <a href="{{ route('follow-along.show', $recitation) }}" class="hidden text-xs font-semibold text-qaari-primary sm:inline hover:text-qaari-accent">
                                {{ __('site.follow_along') }}
                            </a>
                            <form method="POST" action="{{ route('library.favorites.destroy', $recitation) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-xs font-semibold text-qaari-muted hover:text-red-700" title="{{ __('site.remove_favorite') }}">
                                    ♥
                                </button>
                            </form>
                        </li>
